...
const LoginPage = () => {
  ...
  const handleMessege = () => {
    if (error) {
      return (
        <p className="text-red-500 bg-red-200 p-2 px-5 rounded-md">{error}</p>
      );
    } else if (success) {
      return (
        <p className="text-green-500 bg-green-200 p-2 px-5 rounded-md">
          Login Success
        </p>
      );
    }
    ...
    <div className="flex w-full h-screen">
      <div className="w-[60%]">
        <img
          src="LoginBanner.png"
          alt=""
          className="w-full h-screen bg-gradient-to-b from-gray-800 to-blue-600"
        />
      </div>
      <div className="flex flex-col  gap-4  w-[40%]">
        <div className="w-[80%] mx-auto flex flex-col gap-4 justify-center h-full">
          <h1 className="text-2xl font-bold bg-[#0D28A6] p-2 px-5 rounded-md text-white w-max mb-4">
            Logo
          </h1>
          ...
          <button
            ...
            className="bg-[#0D28A6] text-white p-2 text-lg font-medium mt-4 hover:bg-[#1b2b74]"
          >
            Sign In
          </button>
        </div>
